...
### Zoomed in on promoter area
# Pivot dataframe bed_strands_merged for pheatmap
unique(bed_strands_merged$PCR_prod)

combined_data_filtered_pivoted <- bed_strands_merged %>%
  filter(grepl("prom", PCR_prod)) %>%
  select(Anonymized_ID_tissue, start, percentage) %>%
  pivot_wider(names_from = start, values_from = percentage)
...
# Save figure ONT_heatmap_per_patient as .svg
ggsave(
  "figures/ONT_heatmap_per_patient_prom.svg",
  plot = ONT_heatmap_per_patient,
  scale = 1,
  width = 16,
  height = 8,
...
### Zoomed in on lncRNA area

# Pivot dataframe bed_strands_merged for pheatmap
unique(bed_strands_merged$PCR_prod)

combined_data_filtered_pivoted <- bed_strands_merged %>%
  filter(grepl("lncRNA", PCR_prod)) %>%
  select(Anonymized_ID_tissue, start, percentage) %>%
  pivot_wider(names_from = start, values_from = percentage)
...
# Save figure ONT_heatmap_per_patient as .svg
ggsave(
  "figures/ONT_heatmap_per_patient_lncRNA.svg",
  plot = ONT_heatmap_per_patient,
  scale = 1,
  width = 16,
  height = 8,
...
### Zoomed in on 3'UTR area

# Pivot dataframe bed_strands_merged for pheatmap
unique(bed_strands_merged$PCR_prod)

combined_data_filtered_pivoted <- bed_strands_merged %>%
  filter(grepl("UTR", PCR_prod)) %>%
  select(Anonymized_ID_tissue, start, percentage) %>%
  pivot_wider(names_from = start, values_from = percentage)
...
# Save figure ONT_heatmap_per_patient as .svg
ggsave(
  "figures/ONT_heatmap_per_patient_3UTR.svg",
  plot = ONT_heatmap_per_patient,
  scale = 1,
  width = 16,
  height = 8,
